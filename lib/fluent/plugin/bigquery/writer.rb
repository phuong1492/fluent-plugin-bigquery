  ...
  module BigQuery
    class Writer
      ...
      def commit_load_job(chunk_id_hex, response)
        ...
        log.debug "load job finished", id: job_id, state: response.status.state, input_file_bytes: stats && stats.input_file_bytes, input_files: stats && stats.input_files, output_bytes: stats && stats.output_bytes, output_rows: stats && stats.output_rows, bad_records: stats && stats.bad_records, duration: duration.round(2), project_id: project, dataset: dataset, table: table_id
        @num_errors_per_chunk.delete(chunk_id_hex)
      end
